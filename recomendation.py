...
#Exact Question (0-4)
if(lst[0]==1):
    df3=df2.query("brand_name == 'apple'")
elif(lst[0]==2):
    df3=df2.query("brand_name != 'apple'")
else:
    df3=df2
if(lst[1]==2):
    ...
    df3=df3.query(f"brand_name == {user_brand}")
if(lst[2]==1):
    df3=df3.query("screen_size <= 6.1")
elif(lst[2]==2):
    df3=df3.query("screen_size > 6.1 and screen_size <=6.6")
elif(lst[2]==3):
    df3=df3.query("screen_size > 6.6")
if(lst[3]==1):
    df3=df3.query("has_5g == True")
elif(lst[3]==2):
    df3=df3.query("has_5g != False")
budget_min, budget_max = map(int,lst[4].split('-'))
df3=df3.query(f"price >= {budget_min} and price <= {budget_max}")
...
def bad_software(row):
    if row['brand_name'] not in ['apple','google','samsung','motorola','samsung']:
        ...
        return row['rating']
if any(lst[15:17]):
    df3['rating']=df3.apply(bad_software,axis=1)
if lst[14]:
    df3 = df3.query("brand_name == 'google' or brand_name == 'apple' or brand_name == 'samsung'")

#Display Questions (18-21)
def bad_display(row):
    if row['refresh_rate'] < q3_refresh:
        return row['rating']*(0.8+0.2*row['refresh_rate']/q3_refresh)
    else:
        return row['rating']
if any( ([lst[5]] + lst[17:19])):
    df3['rating']=df3.apply(bad_display,axis=1)
if lst[19]:
...
def less_storage(row):
    if row['extended_memory_available'] == 0:
        return row['rating']*0.98
    else:
        return row['rating']
if any( (lst[12:14] + lst[22:24])):
    df3['rating']=df3.apply(less_storage,axis=1)
if lst[24]:
...
def battery_low(row):
    if row['brand_name'] != 'apple':
        if row['battery_capacity'] <q1_battery:
            return row['rating']*(0.98 + 0.02*row['battery_capacity']/q3_battery)
        else:
            return row['rating']
    else:
...
def charging_slow(row):
    if row['fast_charging'] < q1_charging: 
        return row['rating']*(0.985 + 0.015*row['fast_charging']/q3_charging)
    else:
        return row['rating']
    
if (lst[5:8]+lst[13:15]+lst[27:29]).count(True)>=2:
    df3['rating']=df3.apply(battery_low,axis=1)
if lst[29]:
    df3['rating']=df3.apply(charging_slow,axis=1)
